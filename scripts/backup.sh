#!/bin/bash
# TextAgentForU 数据定期备份脚本
# 用法: crontab -e → 0 4 * * * /opt/textagent-all/scripts/backup.sh
#
# 功能:
...
# ---- 配置 ----
PROJECT_DIR="${TEXTAGENT_PROJECT_DIR:-/root/TextAgentForU}"
...
BACKUP_FILE="${BACKUP_DIR}/textagent-data-${TIMESTAMP}.tar.gz"
...
while IFS= read -r old_file; do
    rm -f "$old_file"
    DELETED=$((DELETED + 1))
done < <(find "$BACKUP_DIR" -name "textagent-data-*.tar.gz" -mtime +${KEEP_DAYS} -type f 2>/dev/null)
...
# ---- 统计 ----
TOTAL_BACKUPS=$(find "$BACKUP_DIR" -name "textagent-data-*.tar.gz" -type f | wc -l | tr -d ' ')
