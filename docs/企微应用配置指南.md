...
3. **URL 先不填**，等 TextAgent 服务启动后再回来配置
...
这一步**必须做**，否则 TextAgent 能收到消息但无法回复。
...
2. 确认填的是 TextAgent **运行所在机器**的 IP，不是你自己电脑的 IP
...
部署完成后，你可以直接在**微信聊天界面**中使用 TextAgent，无需安装企业微信客户端。
...
2. 点击进入团队 → 找到 **TextAgentForU**（或你设置的应用名称）
3. 点击进入聊天页面，就可以开始对话了

![微信中使用TextAgent的三步流程](images/企业微信截图_17730531067300.png)

> 💡 **小技巧**：长按 TextAgent 的聊天 → 「置顶聊天」，这样它会一直在微信聊天列表最上方，随时可以发消息记录。
