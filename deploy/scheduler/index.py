# -*- coding: utf-8 -*-
"""
TextAgent 定时调度器（Event 函数）
通过定时触发器调用 TextAgent Web 函数的 /system 端点。
...
# TextAgent Web 函数的公网 URL（通过环境变量配置）
TEXTAGENT_SYSTEM_URL = os.environ.get("TEXTAGENT_SYSTEM_URL", "http://127.0.0.1:9000/system")
...
def main_handler(event, context):
    ...
    # 调用 TextAgent /system 端点
    try:
        resp = requests.post(
            TEXTAGENT_SYSTEM_URL,
            json=payload,
