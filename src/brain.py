...
def _build_state_summary(state):
    ...
    # 数据销毁待确认
    pending_destroy = state.get("pending_destroy", {})
    if pending_destroy.get("pending"):
        parts.append("⚠️ 用户有待确认的数据销毁请求（用户回复「确认销毁」时 → data.destroy, confirm=true）")
...
def _parse_llm_output(text):
    ...
    if start >= 0 and end > start:
        ...
        # 容错：DeepSeek 偶尔输出双大括号 {{...}}，剥离最外层重试
        # 先 strip 空白，避免换行/缩进导致 startswith 匹配失败
